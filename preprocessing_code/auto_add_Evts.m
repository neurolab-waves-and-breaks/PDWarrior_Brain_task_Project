function [] = auto_add_Evts(condition, Data, Const, Paths)
...
% try
    ...
    if numel(Evt.targetChange_time) > numel(Evt.startChange_time)
        Evt.targetChange_time = Evt.targetChange_time(1:end-1);
    end
...
%             figure; plot(currForce); hold on; plot(find(tag_plateau), currForce(tag_plateau), 'x', 'Color', 'r')
            ...
            endErrorWin = 2.5; % 0.5 seconds, previous 0.1
            % endErrorWin = 2.5; % 0.5 seconds, previous 0.1
            % endErrorWin = 2.0; % 0.5 seconds, previous 0.1
            % endErrorWin = 1.5; % 0.5 seconds, previous 0.1
            if (endMvmt(i)+endErrorWin*SR) <= numel(force_smooth)
                Evt.endError(i)   = median(abs(force_smooth(endMvmt(i) : (endMvmt(i)+endErrorWin*SR))'- Evt.newTargetSize(i)));
            else
                Evt.endError(i)   = median(abs(force_smooth(endMvmt(i) : end)'- Evt.newTargetSize(i)));                
            end           
        end
    end
    start = min(startMvmt);
    %% 4 seconds is good, apart from P1 ON meds
    timeWins = 0:4:120;
    for timeWin = 1:numel(timeWins)
        currWin = (start(1) + timeWins(1)*SR) : (start(1) + timeWins(2)*SR);
        if (currWin(end)) < numel(force_smooth)
            Evt.contError(timeWin) = median(abs(force_smooth(currWin)' - targetScale(currWin)));
        end
    end
%     fig_h = figure;

...
def choose_correlation_method(x_data, y_data, var_name, is_predefined_categorical=False):
    ...
    if is_predefined_categorical:
        return 'spearman', 'predefined categorical/ordinal variable'
    ...
    if not x_normal or not y_normal:
        reason = f'non-normal distribution (x_p={x_p:.3f}, y_p={y_p:.3f})'
        return 'spearman', reason
    
    # Both variables are normal, use Pearson
    reason = f'both normally distributed (x_p={x_p:.3f}, y_p={y_p:.3f})'
    return 'pearson', reason
...
def _panel(ax, data, xcol, ycol, title, ylabel, color='orange', is_categorical=False):
    ...
    try:
        x_data = sub[xcol].values
        y_data = sub[ycol].values
        
        method, reason = choose_correlation_method(x_data, y_data, xcol, is_categorical)
        
        if method == 'spearman':
            r, p = spearmanr(x_data, y_data)
            method_label = "ρ"  # Spearman's rho
        else:
            r, p = pearsonr(x_data, y_data)
            method_label = "r"  # Pearson's r
        
        # Display correlation with method indicator
        ax.text(0.68, 0.92, f"{method_label}={r:.2f}\np={p:.3f}", 
                transform=ax.transAxes, va='top', fontsize=10)
        
        # Print diagnostic information
        print(f"{title} - {ylabel} vs {xcol}:")
        print(f"  Method: {method.capitalize()}")
        print(f"  Reason: {reason}")
        print(f"  {method_label}={r:.2f}, p={p:.3f}")
        print()
        
    except Exception as e:
        print(f"Correlation failed for {title} - {ylabel}: {e}")
        pass
...
def plot_2x2_vs_demo(wide_df, demo, color='orange', save_prefix=None):
    ...
    fig.savefig(f'{save_prefix}.png', bbox_inches='tight', pad_inches=0.2, dpi=300)
